...
async function scanWithAI() {
  if (!newRecord.value.image && !imagePreview.value) return;
  aiScanning.value = true;
  // Simulated delay
  await new Promise((r) => setTimeout(r, 1800));
  // Mock filled result
  newRecord.value.component = newRecord.value.component ?? "T-8";
  newRecord.value.part = newRecord.value.part ?? "SA";
  newRecord.value.damageType = newRecord.value.damageType ?? "Crack";
  newRecord.value.assessment = newRecord.value.assessment ?? "fail";
  newRecord.value.notes =
    newRecord.value.notes || "Surface crack along the leading edge.";
  ...
  <div class="flex justify-center max-w-4xl mx-auto">
    <div class="flex flex-col gap-6 w-full">
      ...
      <div v-if="imagePreview" class="flex flex-col gap-2">
        <Button
          :label="aiScanning ? 'Scanning…' : 'Scan with AI'"
          :icon="aiScanning ? 'pi pi-spin pi-spinner' : 'pi pi-sparkles'"
          :disabled="aiScanning"
          severity="info"
          class="w-full"
          @click="scanWithAI"
      ...
      <!-- Details (auto-filled) -->
      <div
        class="flex items-center justify-between flex-wrap gap-4 px-1 w-full"
      >
